...
exports.getMeja = async (request, response) => {
    try {
        /** call meja from db using model */ /** pake await karena findAll promise */
...
/** create an exports func to add new meja */
exports.addMeja = async (request, response) => {
    try {
        /** validate data  */
        let resultValidation = validateMeja(request.body)
        if (resultValidation.status == false) { /** !(membalikkan) jika status bernilai false */
            return response.json({
                status: false,
        ...
        /** insert data mja to db using model */
